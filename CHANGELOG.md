...
- **`@sigx/store`**: migrated to sigx core **`0.12.0`** and **narrowed** the peer range to a single minor — `@sigx/reactivity`/`@sigx/runtime-core` are now `^0.12.0`, down from the widened `>=0.5.0 <0.11.0` band that had accumulated across #50/#56. Store imports only stable low-level symbols (`signal`/`watch`/`batch`/`untrack`/`computed`/`isComputed`, Topic v2 `createTopic`/`createTopicGroup`, `defineFactory`, `getCurrentInstance`, `isLiveClient`/`declareLiveClient` from `@sigx/runtime-core/internals`) plus the SSR state-transfer contract (`stateSerializationPlugin`, `window.__SIGX_ASYNC__`), none of which changed across 0.10→0.12 (0.11 is the namespace-agnostic renderer + coded errors; 0.12 adds the new `@sigx/server` package) — verified with typecheck, tests (136) and build against 0.12.0. The single-minor pin restores core's single-copy guarantee that the widened band gave up: `@sigx/reactivity` keeps reactive state in module-local variables, so a second installed copy silently splits core's singletons. Install alongside the same core minor: `npm i @sigx/store @sigx/runtime-core@^0.12.0 @sigx/reactivity@^0.12.0` (most apps already have core via `sigx`). (#64)

### Changed

- **`@sigx/store`** (build): the core version is now sourced from a pnpm **`catalog:`** in `pnpm-workspace.yaml` (`@sigx/reactivity`/`@sigx/runtime-core`/`@sigx/server-renderer`/`@sigx/vite`/`sigx` at `^0.12.0`), so a framework bump is a one-place edit; `pnpm pack` rewrites `catalog:` to the concrete `^0.12.0` range in the published manifest. Internal only — no effect on consumers. (#64)
