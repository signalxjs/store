# SignalX store — shared agent guide
...
SignalX `store` (`signalxjs/store`) — Reactive store for SignalX. A pnpm monorepo (ESM, `"type": "module"`) with the package under `packages/`. Tech stack: TypeScript (strict), Vitest, oxlint; published to npm under the `@sigx` scope.
...
Never commit straight to `main`.** Repo: `signalxjs/store`, base branch `main`.
...
4. **Open a PR with Copilot as the reviewer.** Reference the issue so it auto-closes
   ...
   ```sh
   gh api --method POST repos/signalxjs/store/pulls/<pr>/requested_reviewers \
...
- `packages/store` → `@sigx/store` — Store framework for SignalX - State management abstraction.
...
- **Before merging a PR with user-facing changes, file an issue on the docs
  ...
  gh issue create --repo signalxjs/signalxjs.github.io \
    ...
    --body "Source: signalxjs/store#<pr>. <What needs documenting, and where on the site.> Not yet released."
  ```
...
4. Adapt the repo-specific sections of `AGENTS.md`: the intro (what the repo is),
   "Build, Test, Lint", and "Packages". Replace every `store` with the repo name.
5. Keep the workflow, worktree, and conventions sections as-is — they are the
   shared standard.
6. Lock down `main`: `node scripts/apply-branch-protection.mjs signalxjs/store`.
